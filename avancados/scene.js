...
echoScene.on('message', ctx => ctx.reply('Apenas Menssagens de texto, por favor'))

let sum = 0
const sumScene = new Scene('sum')
sumScene.enter(ctx => ctx.reply('Entrando em sum Scenen'))
sumScene.leave(ctx => ctx.reply('Saindo em sum Scenen'))
...
bot.on('message', ctx => ctx.reply('Entre com /echo ou com /soma para iniciar...'))
